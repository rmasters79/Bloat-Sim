...
def main():
    ...
    for _ in range(trials):
        ...
        bgs1 = roll_bgs(rng, bgs, bloat_def, BLOAT_SLASH_DEF)
        bloat_def -= bgs1
        total_dmg += bgs1 // 2
        bgs2 = roll_bgs(rng, bgs, bloat_def, BLOAT_SLASH_DEF)
        bloat_def -= bgs2
        total_dmg += bgs2 // 2
        if low_num_trials:
            print(f"BGS 1: {bgs1}\nBGS 2: {bgs2}")
        ...
        if bloat_def > 15:
            bgs3 = roll_bgs(rng, bgs, bloat_def, BLOAT_SLASH_DEF)
            bloat_def -= bgs3
            total_dmg += bgs3 // 2
            necking_hits -= 1
            claw_specs -= 1
        ...
        if bloat_def > 15:
            bgs4 = roll_bgs(rng, bgs, bloat_def, BLOAT_SLASH_DEF)
            bloat_def -= bgs4
            total_dmg += bgs4 // 2
            necking_hits -= 1
            claw_specs -= 1
        ...
        claw_damage = np.sum([roll_claw(rng, claw, bloat_def, BLOAT_SLASH_DEF, low_num_trials) for _ in range(claw_specs)])

        accuracy = calc_hit_chance(scythe, bloat_def, BLOAT_SLASH_DEF)
